...
var generateColumnsFromData = (data, existingColumns, enableColumnResizing = true) => {
...
var EnhancedDataTable = ({
  ...
  const autoGeneratedColumns = useMemo2(() => {
    if (initialColumns.length === 0 && data.length > 0) {
      ...
      return generateColumnsFromData(data, void 0, enableColumnResizing);
    }
    if (initialColumns.length > 0 && data.length > 0) {
      return generateColumnsFromData(data, initialColumns, enableColumnResizing);
    }
    return initialColumns;
  }, [data, initialColumns, enableColumnResizing]);
  const [columns, setColumns] = useState3(autoGeneratedColumns);
  useEffect(() => {
    const updatedColumns = autoGeneratedColumns.map((col) => ({
      ...col,
      resizable: enableColumnResizing && col.resizable !== false
    }));
    setColumns(updatedColumns);
  }, [autoGeneratedColumns, enableColumnResizing]);
